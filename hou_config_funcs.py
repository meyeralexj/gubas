def hou_config_read(filename):
    ...
    if fahnestock_flag==1:
        ...
        if C_flag==0:
            x0[12]=CFG.getfloat("Initial Conditions","B into A (1,1)")
            x0[13]=CFG.getfloat("Initial Conditions","B into A (1,2)")
            x0[14]=CFG.getfloat("Initial Conditions","B into A (1,3)")
            x0[15]=CFG.getfloat("Initial Conditions","B into A (2,1)")
            x0[16]=CFG.getfloat("Initial Conditions","B into A (2,2)")
            x0[17]=CFG.getfloat("Initial Conditions","B into A (2,3)")
            x0[18]=CFG.getfloat("Initial Conditions","B into A (3,1)")
            x0[19]=CFG.getfloat("Initial Conditions","B into A (3,2)")
            x0[20]=CFG.getfloat("Initial Conditions","B into A (3,3)")
        ...
        if Cc_flag==0:
            x0[21]=CFG.getfloat("Initial Conditions","A into N (1,1)")
            x0[22]=CFG.getfloat("Initial Conditions","A into N (1,2)")
            x0[23]=CFG.getfloat("Initial Conditions","A into N (1,3)")
            x0[24]=CFG.getfloat("Initial Conditions","A into N (2,1)")
            x0[25]=CFG.getfloat("Initial Conditions","A into N (2,2)")
            x0[26]=CFG.getfloat("Initial Conditions","A into N (2,3)")
            x0[27]=CFG.getfloat("Initial Conditions","A into N (3,1)")
            x0[28]=CFG.getfloat("Initial Conditions","A into N (3,2)")
            x0[29]=CFG.getfloat("Initial Conditions","A into N (3,3)")
        else:# if user defines euler angles use this rotation matrix definition - MAKE SURE INPUT EULER ANGLES MATCH THEIR DEFINITIONS
    ...
    return(G,n,nA,nB,aA,bA,cA,aB,bB,cB,a_shape,b_shape,rhoA,rhoB,t0,tf,tet_fileA,vert_fileA,tet_fileB,vert_fileB,x0,Tgen,integ,h,tol,out_freq,out_time_name,case,flyby_toggle,sg_toggle,tt_toggle,Mplanet,a_hyp,e_hyp,i_hyp,RAAN_hyp,om_hyp,tau_hyp,sol_rad,au_def,love1,love2,refrad1,refrad2,eps1,eps2,Msun)