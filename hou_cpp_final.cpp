...
void rk4_stack(double t0, double tf, mat x0, double h, parameters inputs, mat* tout, mat* xout, std::function<mat(mat,mat, parameters)> ode);
...
int main() {
    ...
    mat tk(order+1,order/2+2),IA(1,3),C(3,3),Cc(3,3),Cs(3,3),IB(1,3),sg_acc(3,1),tt_1(3,1),tt_2(3,1);
    mat IdA(3,3), IdB(3,3);
    ...
    mat x0, x_sol, t_sol;//set up input and output matrices for integrators
    ...
    if (ics.integ==1){
        cout<<"RK4"<<endl;
        rk4_stack(t0, tf, x0, ics.h, inputs,&t_sol, &x_sol, hou_ode);
    }
    else if (ics.integ==2){
        cout<<"LGVI"<<endl;
...
void rk87(double t0, double tf, mat x0, double rel_tol, parameters inputs, mat* tout, mat* xout, std::function<mat(mat,mat, parameters)> ode){
    ...
    while(t(0,0)<tf){
	    ...
	    throw std::invalid_argument( "RK87 Integration Step Too Small" );
        }
    }
...
mat hou_ode(mat x, mat t, parameters inputs){
    ...
    mat rd=cross(r,wc)+v;
    mat vd=cross(v,wc)-(1./ *(inputs.m))*du_dr+(*(inputs.sg_acc));
    mat Cdc=Cc*wc_tilde;
...
void ic_read(initialization* ics){
    ...
    int order, order_a, order_b, a_shape, b_shape, Tgen, integ, sg_toggle, tt_toggle;
    double G,rhoA,rhoB,aA,bA,cA,aB,bB,cB,t0,tf,h,tol, sol_rad, au_def, love1, love2, refrad1, refrad2, eps1, eps2, Msun;
